...
void cal_Roll_Pitch(mpu6050* __my_mpu6050){
	...
	gyro_roll += gyro_x_dps * dt;
	gyro_pitch += gyro_y_dps * dt;
	gyro_yaw += gyro_z_dps * dt;
	...
	__my_mpu6050->pitch = fused_pitch;
	__my_mpu6050->roll = fused_roll;
	__my_mpu6050->yaw = gyro_yaw;
...
void set_dlpf(I2C_HandleTypeDef* hi2c){
	...
	uint8_t data[2];
	data[0] = ADDRESS_CONFIG;
	for(uint8_t i = 2; i <= 7; i++){
		data[1] = (i << 3 || 0x02);
		HAL_I2C_Master_Transmit(hi2c, slave_address, data, 2, 1000);
	}
}
...
void set_sample_rate(I2C_HandleTypeDef* hi2c, uint16_t sample_rate_you_want) {
	...
	data_to_write[0] = ADDRESS_SAMPLERT_DIV; //접근할 레지스터(SMPLRT_DIV)의 주소
